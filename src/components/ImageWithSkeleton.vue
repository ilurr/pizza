...
<template>
    <div :class="['relative overflow-hidden', wrapperClass]" :style="wrapperStyle">
        <Skeleton v-if="!loaded" :width="skeletonWidth" :height="skeletonHeight" :class="skeletonClass" class="absolute inset-0" />
        <img v-show="loaded" :src="src" :alt="alt" :class="['w-full h-full object-cover transition-opacity duration-300', imgClass]" @load="onLoad" @error="onError" />
    </div>
</template>
