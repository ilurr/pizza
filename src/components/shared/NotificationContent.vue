...
const loadNotifications = () => {
    ...
    const allNotifications = [...stored.map(formatPaymentNotification), ...systemNotifications];

    notifications.value = allNotifications.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
    updateStats();
};
...
<template>
    <div class="notification-content">
        ...
        <div v-if="isDashboard && notifications.length > 0" class="flex gap-2 p-4 justify-end">
            <Button label="Mark All Read" icon="pi pi-check" severity="secondary" @click="markAllAsRead" :disabled="notificationStats.unread === 0" size="small" />
            <Button label="Clear All" icon="pi pi-trash" severity="secondary" @click="clearAll" :disabled="notificationStats.total === 0" size="small" />
        </div>

        <!-- Empty State -->
        <div v-if="notifications.length === 0" class="text-center py-12">
            <i class="pi pi-bell-slash !text-6xl text-gray-300 mb-0"></i>
            <h3 class="text-2xl font-semibold text-gray-600 dark:text-gray-400 mb-2">No notifications</h3>
            <p class="text-gray-500 dark:text-gray-500">Notifications will appear here when you have activity</p>
        </div>
        ...
        <div v-else class="">
            <div
                ...
                :key="notification.id"
                :class="['bg-white dark:bg-neutral-800 p-4 md:p-6 transition-all border-b border-surface', !notification.read ? 'border-l-4 !border-l-blue-500' : 'border-l-4 !border-l-transparent']"
                ...
                <div class="flex items-start space-x-4">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-start justify-between">
                            <div class="flex-1">
                                <h4 :class="['text-sm font-medium mb-1', !notification.read ? 'text-gray-900 dark:text-white' : 'text-gray-700 dark:text-gray-300']">
                                    {{ notification.title }}
                                ...
                                <div v-if="notification.type === 'payment'" class="text-xs text-gray-500 dark:text-gray-500">
                                    <span v-if="notification.external_id">Order ID: {{ notification.external_id }}</span>
                                    <span v-if="notification.amount" class="ml-2"> Amount: {{ NotificationService.formatCurrency(notification.amount) }} </span>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center space-x-2 ml-4">
                                ...
                                <Button v-if="isDashboard" icon="pi pi-times" text size="small" severity="danger" @click.stop="deleteNotification(notification.id)" class="w-6 h-6" />
                            </div>
                        </div>
                    </div>
                </div>
