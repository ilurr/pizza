...
const containerClasses = computed(() => {
    const base = 'flex justify-between items-stretch fixed w-full bottom-0 md:bottom-4 md:rounded-md shadow-[0_-1px_12px_0_rgba(0,0,0,0.1)] md:-translate-x-2/4 md:left-2/4 z-10 md:px-4 bg-white dark:bg-neutral-700 max-w-3xl z-50';
...
<template>
    <div :class="containerClasses">
        <router-link v-for="(item, index) in menuItems" :key="item.path" class="basis-1/5 grow-0 flex justify-center items-center h-full" :to="item.path">
            <button
                type="button"
                :class="[
            ...
            >
                <div class="relative">
                    ...
                    <Badge v-if="showBadge(item)" severity="danger" class="absolute -top-1 -right-2 min-w-[10px] h-[10px] text-[8px]" />
                </div>
