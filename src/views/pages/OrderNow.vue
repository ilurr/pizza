...
const searchNearbyDrivers = async (location) => {
    ...
    try {
        ...
        const res = byKelurahan
            ? await api.drivers.getAvailableDriversByKelurahan(location.kelurahanId)
            : await api.drivers.getAvailableDrivers({ lat: location.lat, lng: location.lng }, 15);
                ...
                <div class="relative p-4 lg:p-0">
                    ...
                    <div class="text-center mb-8" v-if="!userLocation">
                        <ImageWithSkeleton src="/src/assets/images/Orders.svg"
                            wrapperClass="relative mx-auto aspect-square mb-4 md:rounded-xl" width="180px"
                            height="180px" />

                        <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-4">Order Pizza Now! 🍕</h1>

                        <p class="text-gray-600 dark:text-gray-300 mb-6">Tell us where you are and we'll find the
                            nearest pizza chef on wheels!</p>
                        <Button label="Select Your Location" icon="pi pi-map-marker" size="large"
                    ...
                    <div v-if="showDriverList && !isLoading" class="mb-8">
                        ...
                        <div v-if="availableDrivers.length === 0" class="text-center py-12">
                            ...
                            <p class="text-gray-600 dark:text-gray-300 mb-6">No pizza chefs cover your area right now.
                                Try a different address or check back later.</p>
                            <div class="space-y-3">
                                <Button label="Try Different Location" icon="pi pi-refresh"
                                    @click="showLocationPicker" />
                        ...
                        <div v-else>
                            <div class="text-center mb-6">
                                <h2 class="text-2xl font-bold text-gray-900 dark:text-white !mt-8 mb-2">Chefs Who
                                    Deliver to Your Area</h2>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div v-for="driver in availableDrivers" :key="driver.id" :class="[
                                    ...
                                    <div class="flex items-center">
                                        <div class="relative">
                                            <ImageWithSkeleton :src="driver.avatar"
                                                wrapperClass="relative aspect-square rounded-full" width="45px"
                                                height="45px" />
                                        ...
                                        <div class="ml-3 flex-1">
                                            <h3 class="font-semibold text-xl mb-1"
                                                :class="[driver.isAvailable !== false ? 'text-gray-900 dark:text-white' : 'text-gray-500 dark:text-gray-400']">
                                                {{ driver.name }}
                                            </h3>
                                            <div v-if="driver.isAvailable !== false"
                            ...
                            <div v-if="availableDrivers.length > 0 && availableDrivers.every((d) => d.isAvailable === false)"
                                ...
                                <p class="text-yellow-800 dark:text-yellow-200 font-medium mb-0">All chefs in your area
                                    are temporarily unavailable</p>
                                <p class="text-yellow-700 dark:text-yellow-300 text-sm mb-0">Try again later or pick a
                                    different location</p>
                                <Button label="Refresh" icon="pi pi-refresh" size="small" severity="warning"
                                    class="mt-3" @click="searchNearbyDrivers(userLocation)" />
                            </div>
                        </div>
                    </div>
