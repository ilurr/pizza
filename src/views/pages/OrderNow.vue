...
const searchNearbyDrivers = async (location) => {
    ...
    try {
        const res = await api.drivers.getAvailableDrivers({ lat: location.lat, lng: location.lng }, 15);
        const list = res && res.success && res.data && res.data.drivers ? res.data.drivers : [];
        const availableDrivers = list.map((d) => {
            const dist = d.currentLocation ? distanceKm(location.lat, location.lng, d.currentLocation.lat, d.currentLocation.lng) : 0.5;
                ...
                <div class="relative p-4 md:p-0">
                    ...
                    <div v-if="showDriverList && !isLoading" class="mb-8">
                        ...
                        <div v-if="availableDrivers.length === 0" class="text-center py-12">
                            ...
                            <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-2">No Pizza Chefs Found Nearby</h2>
                            <p class="text-gray-600 dark:text-gray-300 mb-6">Sorry, there are no pizza chefs available in your area right now.</p>
                            <div class="space-y-3">
                        ...
                        <div v-else>
                            <div class="text-center mb-6">
                                <h2 class="text-2xl font-bold text-gray-900 dark:text-white !mt-8 mb-2">Pizza Chefs Near You</h2>
                            ...
                            <div v-if="availableDrivers.length > 0 && availableDrivers.every((d) => d.isAvailable === false)" class="text-center mt-6 p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-800">
                                ...
                                <p class="text-yellow-800 dark:text-yellow-200 font-medium mb-0">All nearby chefs are temporarily unavailable</p>
                                <p class="text-yellow-700 dark:text-yellow-300 text-sm mb-0">Please try again later or select a different location</p>
