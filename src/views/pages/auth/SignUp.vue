        ...
        <div class="max-w-xl flex flex-col items-center justify-center mt-20 mb-8 px-4 sm:px-0 z-[2]">
            <div class="w-full bg-white dark:bg-neutral-900 py-6 px-6 sm:px-10 sm:py-10 rounded-xl shadow-lg md:shadow-2xl">
                <div class="flex justify-center flex-col items-center mb-8">
                    ...
                    <span class="text-muted-color font-medium">Daftar dulu, setelah itu kamu bisa panggil papa pizza untuk dateng ke tempatmu!</span>
                </div>

                <form @submit.prevent="handleRegister">
                    <label for="email" class="block text-surface-900 dark:text-surface-0 text-xl font-medium mb-2">Email *</label>
                    <InputText id="email" type="email" placeholder="Your email" class="w-full h-[37px] mb-8" v-model="email" :class="{ 'p-invalid': fieldErrors.email }" />
                    <small v-if="fieldErrors.email" class="p-error block -mt-6 mb-4">{{ fieldErrors.email }}</small>

                    <label for="fullName" class="block text-surface-900 dark:text-surface-0 text-xl font-medium mb-2">Fullname *</label>
                    <InputText id="fullName" type="text" placeholder="Your full name" class="w-full h-[37px] mb-8" v-model="fullName" :class="{ 'p-invalid': fieldErrors.fullName }" />
                    <small v-if="fieldErrors.fullName" class="p-error block -mt-6 mb-4">{{ fieldErrors.fullName }}</small>

                    <label for="whatsapp" class="block text-surface-900 dark:text-surface-0 text-xl font-medium mb-2">No. Whatsapp *</label>
                    <InputText id="whatsapp" type="tel" placeholder="Your WhatsApp number" class="w-full h-[37px] mb-8" v-model="whatsapp" :class="{ 'p-invalid': fieldErrors.whatsapp }" />
                    ...
                    <Password id="confirmPassword" v-model="confirmPassword" placeholder="Confirm your password" :toggleMask="true" class="w-full h-[37px] mb-8" fluid :feedback="false" :invalid="!!fieldErrors.confirmPassword"> </Password>
                    <small v-if="fieldErrors.confirmPassword" class="p-error block -mt-6 mb-4">{{ fieldErrors.confirmPassword }}</small>
                    ...
                    <div class="terms flex items-center justify-between p-2 mb-6 gap-8 border rounded border-gray-500">
                        <ScrollPanel :style="{ width: '100%', height: '200px' }">
                            <ol>
                                <li>Dengan mendaftar di website kami, Anda menyetujui untuk mematuhi dan terikat oleh syarat dan ketentuan berikut, yang dapat kami perbarui dari waktu ke waktu tanpa pemberitahuan sebelumnya.</li>
                                <li>Kami menghargai privasi Anda. Semua informasi pribadi yang dikumpulkan selama proses pendaftaran akan digunakan sesuai dengan kebijakan privasi kami.</li>
                                ...
                                <li>
                                    Kami berhak untuk mengubah syarat dan ketentuan ini kapan saja. Perubahan akan efektif segera setelah diposting di website kami. Penggunaan terus-menerus Anda atas website setelah perubahan diposting akan dianggap
                                    sebagai penerimaan Anda terhadap perubahan tersebut.
                                </li>
                                <li>
                                    Kami berhak untuk mengakhiri atau menangguhkan akses Anda ke website kami tanpa pemberitahuan sebelumnya atau kewajiban apa pun, jika Anda melanggar syarat dan ketentuan ini atau terlibat dalam aktivitas yang
                                    dianggap tidak sesuai oleh kami.
                                </li>
                            </ol>
                            <Checkbox v-model="acceptedTerms" id="terms" binary class="mr-2" :class="{ 'p-invalid': fieldErrors.terms }"></Checkbox>
