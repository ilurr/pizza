            ...
            <div class="w-full bg-white dark:bg-neutral-900 py-6 px-6 sm:px-10 sm:py-10 rounded-xl shadow-lg md:shadow-2xl">
                ...
                <div class="flex justify-center flex-col items-center mb-8">
                    ...
                    <span class="text-muted-color font-medium text-center">
                        {{ isSuccess ? 'We have sent password reset instructions to your email address.' : "Enter your email address and we'll send you instructions to reset your password." }}
                    </span>
                </div>
                ...
                <div v-if="isSuccess" class="text-center">
                    <div class="mb-6">
                        ...
                        <p class="text-surface-600 dark:text-surface-400 mb-2">Password reset email sent to:</p>
                    ...
                    <div class="space-y-4">
                        <p class="text-sm text-surface-600 dark:text-surface-400">Didn't receive the email? Check your spam folder or try again.</p>
                ...
                <form v-else @submit.prevent="handleForgotPassword">
                    ...
                    <InputText id="email" type="email" placeholder="Enter your email address" class="w-full mb-8" v-model="email" :class="{ 'p-invalid': fieldErrors.email }" :disabled="isLoading" />
                    <small v-if="fieldErrors.email" class="p-error block -mt-6 mb-4">
                        {{ fieldErrors.email }}
                    </small>

                    <Button label="Send Reset Instructions" class="w-full mb-4" type="submit" :loading="isLoading" :disabled="isLoading" />

                    <Message v-if="errorMessage" severity="error" variant="simple" size="small" class="w-full flex justify-center px-2 py-1 mb-4">
                        {{ errorMessage }}
                    </Message>
                </form>

                <!-- Footer Links -->
                <Divider layout="horizontal" class="flex !mt-6 !mb-8"> Remember your password? </Divider>
