    ...
    <div class="p-4">
        <div class="flex flex-wrap items-center justify-between gap-3 mb-4">
            <div class="flex flex-row items-center justify-between gap-3 mb-4">
                ...
                <Button label="Refresh" icon="pi pi-refresh" outlined size="small" :loading="isLoading" @click="loadEarnings" />
            </div>
        ...
        <template v-else>
            ...
            <div class="grid grid-cols-2 gap-4 w-full mb-6">
                <div v-for="s in reportSummary" :key="s.title" class="rounded-xl border-2 bg-white dark:bg-neutral-800 border-surface-200 dark:border-surface-700 overflow-hidden transition-shadow hover:shadow-sm">
                    <div class="p-5 flex flex-col items-center text-center gap-3">
                        ...
                        >
                            <i :class="[s.icon, '!text-2xl shrink-0', s.color === 'blue' && 'text-blue-500', s.color === 'green' && 'text-green-500']"></i>
                        </span>
                        <div class="min-w-0 w-full">
                            ...
                            <div class="text-surface-700 dark:text-surface-300 font-medium text-sm mt-1">{{ s.title }}</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div v-if="dailyDeposits.length === 0" class="rounded-xl border-2 border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-800/50 p-8 text-center text-600">
                    <i class="pi pi-inbox text-4xl mb-3 block"></i>
                    <p class="mb-0">No deposit rows for this period.</p>
                </div>
                <div v-else class="rounded-xl border-2 border-surface-200 dark:border-surface-700 bg-white dark:bg-neutral-800 overflow-hidden">
                    <DataTable :value="dailyDeposits" responsiveLayout="scroll" class="p-datatable-sm" :paginator="dailyDeposits.length > 10" :rows="10">
                        ...
                        <Column header="Total earning" style="min-width: 9rem">
                            <template #body="{ data }">
                                <span class="font-semibold text-green-600 dark:text-green-400">{{ formatCurrency(data.totalEarning) }}</span>
                            </template>
                        </Column>
